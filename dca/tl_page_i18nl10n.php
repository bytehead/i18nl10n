...
(
  'default' =>   '{menu_legend},title,language;
...
class tl_page_i18nl10n extends Backend
{
    ...
    $label = '<span style="color:#b3b3b3; padding-left:3px;">
                  <img style="vertical-align:middle" src="system/modules/i18nl10n/html/flag_icons/png/'.$row['language'].'.png" /> 
	...
	public function localize_all() 
  {
      ...
      {
        $flag = '<img style="vertical-align:middle" src="system/modules/i18nl10n/html/flag_icons/png/'.$GLOBALS['TL_CONFIG']['i18nl10n_default_language'].'.png" /> ';
        $GLOBALS['TL_DCA']['tl_page']['list']['sorting']['breadcrumb'] .=   '<form method="post" action="contao/main.php?do=i18nl10n">
                                                                                <div id="i18nl10n_localise_all_confirm">'.
                                                                                    sprintf($GLOBALS['TL_LANG']['tl_page_i18nl10n']['localize_all_q'],$flag.$GLOBALS['TL_LANG']['LNG'][$GLOBALS['TL_CONFIG']['i18nl10n_default_language']])
                                                                                    .'<div class="tl_submit_container">
                                                                                        <input type="submit" value="'.utf8_ucfirst($GLOBALS['TL_LANG']['MSC']['yes']).'" class="tl_submit" name="localize_all_" />
                                                                                        <a href="contao/main.php?do=i18nl10n">'.utf8_ucfirst($GLOBALS['TL_LANG']['MSC']['no']).'</a>
                                                                                    </div>
                                                                                </div>
      ...
      {                
        ...
        {
          $SQL =  "INSERT INTO tl_page_i18nl10n (pid,sorting,tstamp,language,title,type,pageTitle,description,cssClass,published,start,stop,dateFormat,timeFormat,datimFormat)
                  SELECT p.id AS pid, p.sorting, p.tstamp, '$l' AS language,p.title, p.type, p.pageTitle, p.description, p.cssClass, p.published, p.start, p.stop, p.dateFormat, p.timeFormat, p.datimFormat
                  FROM tl_page p LEFT JOIN tl_page_i18nl10n i ON p.id = i.pid AND i.language='$l' WHERE p.language='".$GLOBALS['TL_CONFIG']['i18nl10n_default_language']."' and p.type !='root' AND i.pid IS NULL";
           //echo $SQL;

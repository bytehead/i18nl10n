...
$i18nl10n_default_language = $this->Database->prepare("SELECT language FROM tl_page WHERE type='root' AND published=1 ORDER BY sorting")->limit(1)->execute()->language;
...
(
  ...
  'eval'          => array('mandatory'=>true,'style'=>'width:2em;','tl_class'=>'w50','nospace' => true),
  'save_callback' => array(array('tl_settings_l10ns','ensureUnique'),array('tl_settings_l10ns','ensureExists')) 
);
...
(
  ...
  'options'       => array($i18nl10n_default_language =>$GLOBALS['TL_LANG']['LNG'][$i18nl10n_default_language]), 
...
(
  ...
  'save_callback' => array(array('tl_settings_l10ns','ensureOthersUnchecked'))
);
...
{
  public function ensureUnique($languages, DataContainer $dc) 
  {
      return serialize( array_unique( deserialize( $languages ) ) );
  }

  public function ensureExists($languages, DataContainer $dc) 
  {      
    ...
    {
      $dc->addErrorMessage($GLOBALS['TL_LANG']['tl_settings']['i18nl10n_defLangMissingError']
      );
      return false;
    }
  ...
  public function ensureOthersUnchecked($value, DataContainer $dc)
  {
    if($dc->field == 'i18nl10n_alias_suffix' && $value) 
    {
      if($GLOBALS['TL_CONFIG']['addLanguageToUrl'] || $GLOBALS['TL_CONFIG']['i18nl10n_addLanguageToUrl'])
      {
        $dc->addErrorMessage($GLOBALS['TL_LANG']['tl_settings']['i18nl10n_alias_suffixError']);
        $dc->log($GLOBALS['TL_LANG']['tl_settings']['i18nl10n_alias_suffixlError'],__METHOD__, TL_CONFIGURATION);
        return false;
      }
    }
    elseif($dc->field == 'i18nl10n_addLanguageToUrl' && $value)
    {
      if($GLOBALS['TL_CONFIG']['addLanguageToUrl']||$GLOBALS['TL_CONFIG']['i18nl10n_alias_suffix'])
      {
        $dc->addErrorMessage($GLOBALS['TL_LANG']['tl_settings']['i18nl10n_addLanguageToUrlError']);
        $dc->log($GLOBALS['TL_LANG']['tl_settings']['i18nl10n_addLanguageToUrlError'],__METHOD__, TL_CONFIGURATION);
        return false;
      }
    }  
  return $value;
  ...
  public function showReadOnlyField(DataContainer $dc) 
  {
    ...
    {
      return '<h3>
                <label for="ctrl_'.$dc->field.'">'.$GLOBALS['TL_DCA'][$dc->table]['fields'][$dc->field]['label'][0].'</label>
              </h3>
              <input type="hidden" readonly="readonly" name="'.$dc->field.'" value="'.$dc->value.'" />'.$GLOBALS['TL_LANG']['LNG'][$dc->value]." ($dc->value)"
              .'<p class="tl_help tl_tip">'.$GLOBALS['TL_DCA'][$dc->table]['fields'][$dc->field]['label'][1].'</p>';
    }
  }
}

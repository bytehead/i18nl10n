...
$GLOBALS['TL_DCA']['tl_page']['config']['onload_callback'][]   = array('tl_page_l10ns', 'setDefaultLanguage');
$GLOBALS['TL_DCA']['tl_page']['config']['onsubmit_callback'][] = array('tl_page_l10ns', 'generatePageL10n');
...
class tl_page_l10ns extends Backend 
{
   ...
   */
   public function generatePageL10n(DataContainer $dc) 
   {
      ...
      if (!$new_records || (is_array($new_records[$dc->table]) && !in_array($dc->id, $new_records[$dc->table]))) return;
      ...
      foreach ($site_langs as $l) 
      {
         ...
         $fields['sorting'] +=128;
         $fields['language'] = $l; 
         $this->Database->prepare('INSERT INTO tl_page_i18nl10n %s')->set($fields)->execute();
      }
   }
}//end class
